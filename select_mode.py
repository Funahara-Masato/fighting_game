...
def select_mode():
    ...
    jp_info  = pygame.font.Font("meiryo.ttf", 16)
    ...
    while run:
        ...
        info_lines = [
            ("PvP 操作方法", TEXT_LT),
            ("P1（赤）   移動: A / D\u3000\u3000ジャンプ: W\u3000\u3000攻撃: S（長押し→ゴムゴムのピストル）\u3000ガード: Q", TEXT_DIM),
            ("P2（青）   移動: ← / →\u3000ジャンプ: ↑\u3000\u3000攻撃: ↓（長押し→ゴムゴムのピストル）\u3000ガード: RShift", TEXT_DIM),
            ("共通: 同方向2連打でダッシュ / 空中で攻撃ボタン→キック（地上攻撃を回避可）", TEXT_DIM),
        ]
        for i, (line, col) in enumerate(info_lines):
            surf = jp_info.render(line, True, col)
            WIN.blit(surf, (44, info_y + i * 24))
